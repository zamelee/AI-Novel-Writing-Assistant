...
新增伏笔兑现审计入口：自动导演新增一条不消耗 token 的轻量审计通道，可单独入队执行，会扫描《伏笔账本》里所有"待兑现"和"逾期"状态的项目，统计数量并把最近一次审计结果写回任务快照，inspector 抽屉可以直接看到当前还有多少伏笔没兑现。前端审核和工作流水牌从此有了客观的伏笔读数，不必再依赖 AI 评审员口头复述。
- 审计只读取《伏笔账本》数据库并把结果写回任务快照，不会调用 LLM，也不会中断或阻塞自动导演流程，适合在长链写作任务进行中插队做一次轻量检查。
- inspector 快照新增"待兑现伏笔"字段，显示待兑现和逾期计数，以及前 5 条最关键的伏笔标题，方便用户快速定位问题。
- 当这部书还没有任何任务写过审计结果时，inspector 会显示空状态；首次审计由用户在驾驶舱或工作流水牌主动触发，之后会自动保留最近一次结果。
- inspector 抽屉右上角新增“运行伏笔兑现审计”按钮，点了之后会入队一次 audit_foresight_payoff 命令并刷新结果；按钮与「释放锁」按钮并排放置，遵循工作流水牌统一操作风格。
- inspector 抽屉新增「待兑现伏笔」面板：显示待兑现 / 逾期计数、上次审计时间，以及前 5 条最关键的伏笔标题和目标章序；面板中 overdue 状态会用警示色高亮提示。
